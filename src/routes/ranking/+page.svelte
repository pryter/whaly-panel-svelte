<script lang="ts">
  import {getFirestore, collection, getDoc, doc} from "firebase/firestore"
  import {fly} from "svelte/transition";
  import RankingBar from "$lib/components/RankingBar.svelte";
  import {cubicOut} from "svelte/easing";
  import {getContext} from "svelte";

  type RankingItem = Omit<ChartData, "portion">
  type ChartData = {
    playCount: number,
    portion: number,
    title: string
    url: string
  }

  let rankingData: {topFive: ChartData[], other: RankingItem[], date: string} = $state({topFive: [], other: [], date: ""})
  function zeroPad(input: number) {
    let num = input.toString()
    while (num.length < 2) num = `0${num}`
    return num
  }

  const month = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
  const fetchData = async () => {
      const exp = localStorage.getItem("ranking-cache-expires")
      const cache = localStorage.getItem("ranking-cache")
      let data: any
      // expired
      if (!exp || parseInt(exp, 10) < new Date().getTime() || !cache) {
        const firestore = getFirestore()
        const col = collection(firestore, "indexed")
        const docu = await getDoc(doc(col, "top50"))
        data = docu.data()

        localStorage.setItem("ranking-cache", JSON.stringify(data))
        const currentt = new Date(data.time)
        // next 30 min cache should expires
        const hardLimit = currentt.getTime() + 4 * 60 * 60 * 1000
        const min10 = new Date().getTime() + 10 * 60 * 1000
        localStorage.setItem(
          "ranking-cache-expires",
          (min10 > hardLimit ? hardLimit : min10).toString(10)
        )
      } else {
        // load cache
        data = JSON.parse(cache)
      }

      if (!data) return

      const arr: any[] = data.ranking

      const sorted = arr.sort((a, b) => b.playCount - a.playCount)

      const topFive = sorted.slice(0, 5)
      const leftOver = sorted.slice(5, sorted.length)

      topFive.forEach((i: any, k) => {
        const highest = topFive[0].playCount
        topFive[k].portion = (i.playCount / highest) * 100
      })
      const date = new Date(data.time)
      const datString = `${zeroPad(date.getHours())}:${zeroPad(
        date.getMinutes()
      )} ${month[date.getMonth()]} ${date.getDate()} ${date.getFullYear()}`
     rankingData = {topFive, other: leftOver, date: datString}
    }

    let ready = $state(false)
  $effect(() => {
    ready = true
    fetchData()
  })

  type RankedListProps = {
    item: RankingItem,
    index: number
  }

</script>

{#snippet rankedList(props: RankedListProps)}
    <a href={props.item.url} target="_blank" class="flex items-center text-gray-200 cursor-pointer hover:underline">
        <h1 class="mr-2 text-lg text-gray-400">{props.index + 6}</h1>
        <span class="w-full truncate break-all text-sm text-gray-400 sm:text-base">
            {props.item.title}
        </span>
        <h1 class="ml-2 text-right">{props.item.playCount}</h1>
    </a>
{/snippet}

<div class="flex h-screen flex-col items-center justify-center bg-gradient-to-tr from-gray-700 via-gray-800 to-gray-900 overflow-hidden px-6 sm:px-2">
    {#if ready}
        <div out:fly={{ x: 200, duration: 500, easing: cubicOut }}
             in:fly={{ x: -200, duration: 500, easing: cubicOut, delay: 1000 }} class="flex flex-col w-full max-w-xl">
            <a href="/" class="button text-white w-fit flex flex-row pl-2 pr-3 cursor-pointer items-center space-x-2 border-white border rounded-full">
                <svg class="size-[20px]" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    <g id="Iconly/Light-Outline/Arrow---Left" stroke="none" stroke-width="1.5" fill="none" fill-rule="evenodd">
                        <g id="Arrow---Left" transform="translate(11.500000, 12.500000) rotate(-270.000000) translate(-11.500000, -12.500000) translate(4.500000, 4.000000)" fill="currentColor" fill-rule="nonzero">
                            <path d="M6.7743,0 C7.15399577,0 7.46779096,0.282153882 7.51745338,0.648229443 L7.5243,0.75 L7.52369999,13.934 L12.2672291,9.17101315 C12.559491,8.8774899 13.0343636,8.87646731 13.3278869,9.16872914 C13.5947262,9.43442171 13.6198294,9.8510323 13.4026077,10.1451122 L13.3301709,10.2293869 L7.30617086,16.2793869 C7.26764619,16.3180778 7.22595266,16.3516842 7.18192314,16.3802061 C7.16866934,16.388262 7.15511061,16.3964623 7.14128041,16.4042367 C7.12942435,16.4113875 7.11711275,16.4178493 7.10466728,16.4239454 C7.08655679,16.4324122 7.06784795,16.4406833 7.04875665,16.4481939 C7.03403731,16.4542967 7.01972109,16.4593926 7.00529186,16.4640405 C6.98739762,16.4695662 6.96832963,16.4749347 6.9489704,16.4795527 C6.93545418,16.4829859 6.9224453,16.4857056 6.90938559,16.4880773 C6.88995431,16.4914125 6.86984664,16.4942514 6.84949961,16.496277 C6.83398162,16.498026 6.81860624,16.4990792 6.80321635,16.4996611 C6.79388374,16.4998115 6.78411388,16.5 6.7743,16.5 L6.74524677,16.4996251 C6.73052671,16.4990502 6.71582035,16.4980441 6.70114827,16.4966069 L6.7743,16.5 C6.72697147,16.5 6.68066686,16.4956161 6.63577008,16.4872322 C6.62504705,16.4852879 6.6140387,16.482972 6.60307688,16.4804067 C6.58049893,16.475079 6.55875812,16.4689204 6.5374423,16.4618312 C6.52685318,16.4583516 6.51553869,16.4542993 6.5043075,16.4499668 C6.48174039,16.4412024 6.46022651,16.431675 6.43927187,16.4211951 C6.42941864,16.4163265 6.41910264,16.410874 6.40889183,16.4051642 C6.392234,16.3957917 6.37640388,16.3861371 6.36097334,16.3759271 C6.35005285,16.3687237 6.33868575,16.360746 6.32749818,16.3524163 L6.31885445,16.3459258 C6.29245569,16.3257196 6.26743874,16.3037989 6.24396991,16.2803301 L6.24327306,16.279431 L0.218273061,10.229431 C-0.074013127,9.93593197 -0.0730299537,9.46105925 0.22046904,9.16877306 C0.487286308,8.90305834 0.903999213,8.879715 1.19715903,9.09817688 L1.28112694,9.17096904 L6.02369999,13.932 L6.0243,0.75 C6.0243,0.335786438 6.36008644,0 6.7743,0 Z" id="Combined-Shape"></path>
                        </g>
                    </g>
                </svg>
                <h1 class="mt-0.5">Back</h1>
            </a>
            <div class="w-full mt-6">
                <h1 class="text-2xl font-bold text-gray-100">
                    Anonymous Global <br class="sm:hidden"/> Listening Trends
                </h1>
                <p class="text-gray-100">
                    Top tracks ranked by total anonymous plays across the platform.
                </p>
                <p class="text-gray-100 mt-2 sm:mt-0">
                    <span class="text-sm sm:text-base">Last update: <span class="text-gray-400">{rankingData.date}</span></span>
                </p>
                <p class="mb-8 text-xs text-gray-500 sm:mb-10 sm:text-sm">
                    Note: We never store any detail about your server or your command
                    usage.
                </p>
            </div>
            <div class="w-full">
                {#each rankingData.topFive as item, i}
                    <RankingBar index={i} count={item.playCount} url={item.url} title={item.title} progress={item.portion} />
                {/each}
            </div>
            <div class="mt-12 flex h-48 flex-col space-y-2 overflow-y-scroll rounded-xl bg-gray-700 px-6 py-4 sm:h-64 w-full">
                {#each rankingData.other as item, i}
                    {@render rankedList({item, index: i})}
                {/each}
            </div>
        </div>
    {/if}
</div>